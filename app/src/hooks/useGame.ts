...
export function useGame() {
  ...
  const movePlayer = useCallback((direction: Direction): boolean => {
    if (!state.grid || !state.isAlive || state.status !== MatchStatus.Active) {
      return false
    }
  ...
  const checkExit = useCallback((pos: Position) => {
    setState(prev => {
      ...
      if (pos.x !== prev.exitPosition.x || pos.y !== prev.exitPosition.y) return prev
  ...
  const movePlayerOnChain = useCallback(async (direction: Direction) => {
    if (!state.matchId || !connected) return

    try {
      await clientRef.current.movePlayer(state.matchId, direction)
    } catch (err) {
